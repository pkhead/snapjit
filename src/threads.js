...
Process.prototype.runStep = function (deadline) {
    ...
    if (this.enableCompiling) {
        if (!this.compiled) {
            try {
                this.compiled = Compiler.compile(this.topBlock, this.blockReceiver(), this.context.inputs);
                this.coro = this.compiled(this);
            } catch(err) {
                if (Array.isArray(err)) {
                    this.handleError(err[0], err[1]);
    ...
    while (!this.readyToYield && !this.isInterrupted
        ...
        if (this.enableCompiling) {
            if (this.context._yieldEnd && Date.now() < this.context._yieldEnd) {
                this.readyToYield = true;
            } else if (this.coro) {
                this.context._yieldEnd = null;
                let yieldRes;

                try {
                    yieldRes = this.coro.next();
                } catch(err) {
                    if (Array.isArray(err)) {
                        this.handleError(err[0], err[1]);
                        this.readyToTerminate = true;
                ...
                if (yieldRes) {
                    if (yieldRes.done) {
                        console.log("return " + yieldRes.value);
                        this.context.addInput(yieldRes.value);
                        this.coro = null;
                        this.readyToYield = true;
                        this.readyToTerminate = true;
                    } else {
                        this.readyToYield = !this.isAtomic || this.currentTime > deadline;
                        let yieldData = yieldRes.value;

                        if (yieldData) {
                            if (yieldData[0] === "wait") {
                                this.context._yieldEnd = Date.now() + yieldData[1]*1000;
                            }
                        }
                    }
                }
